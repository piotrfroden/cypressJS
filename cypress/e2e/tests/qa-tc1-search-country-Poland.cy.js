...
const jsonData = require("../../fixtures/qa-tc1-parameters.json")

describe('search vehicles in country Poland', () => {
    var date = new Date()
    ...
    it(jsonData.test1.testname, () => {
      ...
      //let strCountry = 'Poland', strCity = 'Cracow', strModel = 'Volkswagen', strCompany = 'Dawson PLC', strCarModel = 'Volkswagen Tiguan', strLicensePlate = '9873', strPrice = '445$', strPricePerDay = '89$'
    ...
    it(jsonData.test2.testname, () => {
      searchPage.checkAlertMessage()
      //let strCountry = 'Poland', strCity = 'Wroclaw', strModel = 'Toyota', strCompany = 'Kaiser, Williams and Arellano', strCarModel = 'Toyota Aygo', strLicensePlate = '181CMD', strPrice = '295$', strPricePerDay = '59$'
      //fill search form with input country, city, model, date pickup and date drop off
      cy.searchVehicles(jsonData.test2.country, jsonData.test2.city, jsonData.test2.model, datePickUp, targetDate)
      //search on results table one vehicle
      cy.rentCar(jsonData.test2.company, jsonData.test2.carModel, jsonData.test2.licensePlate, jsonData.test2.price, jsonData.test2.pricePerDay)
      //after click Rent button on results table selected row try to verify selected data on rent page
      cy.verifySelectedCar(jsonData.test2.carModel, jsonData.test2.company, jsonData.test2.pricePerDay, jsonData.test2.country, jsonData.test2.city, jsonData.test2.licensePlate, datePickUp, targetDate)
      //click rent button
      rentPage.getButtonRent().should('be.visible').click()
      //fill personal data
      cy.summaryRentCar(testUser.userdata.firstname, testUser.userdata.lastname, testUser.userdata.cardNumber, testUser.userdata.email)
    ...
    it(jsonData.test6.testname, () => {
      searchPage.checkAlertMessage()
      cy.searchVehicles(jsonData.test6.country, jsonData.test6.city, jsonData.test6.model, datePickUp, targetDate)
      searchPage.checkMessageEmptyCarsList()
    })
})